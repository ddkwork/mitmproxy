...
func (cw *chunkedWriter) Write(data []byte) (n int, err error) {
	...
	if _ = mylog.Check2(fmt.Fprintf(cw.Wire, "%x\r\n", len(data))); err != nil {
		return 0, err
	}
	if n = mylog.Check2(cw.Wire.Write(data)); err != nil {
		return
	}
	if n != len(data) {
	...
	if _ = mylog.Check2(io.WriteString(cw.Wire, "\r\n")); err != nil {
		return
	}
	if bw, ok := cw.Wire.(*FlushAfterChunkWriter); ok {
		mylog.Check(bw.Flush())
	}
	return
}

func (cw *chunkedWriter) Close() error {
	mylog.Check2(io.WriteString(cw.Wire, "0\r\n"))
	return nil
}
