...
func TestChunk(t *testing.T) {
	...
	data := mylog.Check2(io.ReadAll(r))
...
//		n := mylog.Check2(io.ReadFull(r, readBuf))
//		if n != len(readBuf)-1 {
...
//		got := mylog.Check2(parseHexUint([]byte(tt.in)))
//		if tt.wantErr != "" {
...
func TestChunkReadingIgnoresExtensions(t *testing.T) {
	...
	data := mylog.Check2(io.ReadAll(NewChunkedReader(strings.NewReader(in))))
...
//	readBuf := make([]byte, 7)
//	n := mylog.Check2(cr.Read(readBuf))
//
//	want := "1234567"
...
//		pw.Write([]byte("xx"))
//	}()
//	_ = mylog.Check2(cr.Read(readBuf))
...
//		if _ := mylog.Check2(io.ReadAll(r)); err != io.ErrUnexpectedEOF {
...
func TestChunkEndReadError(t *testing.T) {
	...
	if _, e := (io.ReadAll(r)); e != readErr {
		t.Errorf("expected %v, got %v", readErr, e)
	}
}
