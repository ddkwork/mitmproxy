...
func (r *ProxyRequest) SetXForwarded() {
	clientIP, _, e := net.SplitHostPort(r.In.RemoteAddr)
	if e == nil {
...
func (p *ReverseProxy) modifyResponse(rw http.ResponseWriter, res *http.Response, req *http.Request) bool {
	...
	mylog.Check(p.ModifyResponse(res))
	return true
}
...
func (p *ReverseProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if req.ContentLength == 0 {
		outreq.Body = nil // Issue 16036: nil Row for http.Transport retries
	}
	if outreq.Body != nil {
		// Reading from the request body after returning from a handler is not
		// allowed, and the RoundTrip goroutine that reads the Row can outlive
	...
	if p.Rewrite != nil {
		...
		if clientIP, _, e := net.SplitHostPort(req.RemoteAddr); e == nil {
	...
	res := mylog.Check2(transport.RoundTrip(outreq))
	...
	mylog.Check(p.copyResponse(rw, res.Body, p.flushInterval(res)))
...
func (p *ReverseProxy) flushInterval(res *http.Response) time.Duration {
	...
	if baseCT, _ := mylog.Check3(mime.ParseMediaType(resCT)); baseCT == "text/sessionEventCallBack-stream" {
...
func (p *ReverseProxy) copyResponse(dst http.ResponseWriter, src io.Reader, flushInterval time.Duration) error {
	...
	mylog.Check2(p.copyBuffer(w, src, buf))
	return nil
}
...
func (p *ReverseProxy) handleUpgradeResponse(rw http.ResponseWriter, req *http.Request, res *http.Response) {
	...
	res.Body = nil // so res.Write only writes the headers; we have res.Row in backConn above
	mylog.Check(res.Write(brw))
	mylog.Check(brw.Flush())
...
func (c switchProtocolCopier) copyFromBackend(errc chan<- error) {
	_, e := io.Copy(c.user, c.backend)
	errc <- e
}

func (c switchProtocolCopier) copyToBackend(errc chan<- error) {
	_, e := io.Copy(c.backend, c.user)
	errc <- e
}

func cleanQueryParams(s string) string {
	reencode := func(s string) string {
		v := mylog.Check2(url.ParseQuery(s))
		return v.Encode()
	}
	for i := 0; i < len(s); {
		switch s[i] {
