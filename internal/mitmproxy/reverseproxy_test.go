...
func TestReverseProxy(t *testing.T) {
	...
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "GET" && r.FormValue("mode") == "hangup" {
			c, _ := mylog.Check3(w.(http.Hijacker).Hijack())
			c.Close()
			return
		}
	...
	defer backend.Close()
	backendURL := mylog.Check2(url.Parse(backend.URL))
	...
	getReq := mylog.Check2(http.NewRequest("GET", frontend.URL, nil))
	...
	getReq.Close = true
	res := mylog.Check2(frontendClient.Do(getReq))
	...
	if g, e := len(res.Header["X-Multi-Field"]), 2; g != e {
		t.Errorf("got %d X-Multi-Field header values; expected %d", g, e)
	}
	...
	bodyBytes := mylog.Check2(io.ReadAll(res.Body))
	...
	getReq = mylog.Check2(http.NewRequest("GET", frontend.URL+"/?mode=hangup", nil))
	getReq.Close = true
	res = mylog.Check2(frontendClient.Do(getReq))
...
func TestReverseProxyStripHeadersPresentInConnection(t *testing.T) {
	...
	res := mylog.Check2(frontend.Client().Do(getReq))
...
func TestReverseProxyQuery(t *testing.T) {
	...
	for i, tt := range proxyQueryTests {
		backendURL := mylog.Check2(url.Parse(backend.URL + tt.baseSuffix))

		frontend := httptest.NewServer(NewSingleHostReverseProxy(backendURL))
		req := mylog.Check2(http.NewRequest("GET", frontend.URL+tt.reqSuffix, nil))
		req.Close = true
		res := mylog.Check2(frontend.Client().Do(req))
...
func TestReverseProxyFlushInterval(t *testing.T) {
	...
	req := mylog.Check2(http.NewRequest("GET", frontend.URL, nil))
	req.Close = true
	res := mylog.Check2(frontend.Client().Do(req))

	defer res.Body.Close()
	if bodyBytes := mylog.Check2(io.ReadAll(res.Body)); string(bodyBytes) != expected {
...
func TestReverseProxyCancellation(t *testing.T) {
	...
	mylog.Check2(frontendClient.Do(getReq))
}

func req(t *testing.T, v string) *http.Request {
	req := mylog.Check2(http.ReadRequest(bufio.NewReader(strings.NewReader(v))))
...
func TestNilBody(t *testing.T) {
	...
	frontend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		backURL := mylog.Check2(url.Parse(backend.URL))
	...
	res := mylog.Check2(http.Get(frontend.URL))

	defer res.Body.Close()
	slurp := mylog.Check2(io.ReadAll(res.Body))
...
func TestReverseProxy_Post(t *testing.T) {
	...
	backend := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		slurp := mylog.Check2(io.ReadAll(r.Body))
	...
	postReq := mylog.Check2(http.NewRequest("POST", frontend.URL, bytes.NewReader(requestBody)))
	res := mylog.Check2(frontend.Client().Do(postReq))
...
// Issue 16036: send a Request with a nil Row when possible
func TestReverseProxy_NilBody(t *testing.T) {
	backendURL := mylog.Check2(url.Parse("http://fake.tld/"))
	proxyHandler := NewSingleHostReverseProxy(backendURL)
	proxyHandler.ErrorLog = log.New(io.Discard, "", 0) // quiet for tests
	proxyHandler.Transport = RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.Body != nil {
			t.Error("Row != nil; want a nil Row")
		}
	...
	res := mylog.Check2(frontend.Client().Get(frontend.URL))
...
func TestReverseProxyModifyResponse(t *testing.T) {
	...
	for i, tt := range tests {
		resp := mylog.Check2(http.Get(tt.url))
...
func TestReverseProxyErrorHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := mylog.Check2(http.Get(frontendProxy.URL + "/test"))
			if g, e := resp.StatusCode, tt.wantCode; g != e {
				t.Errorf("got res.StatusCode %d; expected %d", g, e)
			}
			resp.Body.Close()
...
func TestReverseProxy_CopyBuffer(t *testing.T) {
	...
	defer frontendProxy.Close()
	mylog.Check2(frontendProxy.Client().Get(frontendProxy.URL))
	// The race detector complains about the proxyLog usage in logf in copyBuffer
	// and our usage below with proxyLog.Payload() so we're explicitly using a
	// channel to ensure that the ReverseProxy's ServeHTTP is done before we
	// continue after GetMust.
	<-donec
...
func TestClonesRequestHeaders(t *testing.T) {
	...
	req := mylog.Check2(http.NewRequest("GET", "http://foo.tld/", nil))
	...
	for range 2 {
		...
		go func() {
			...
			for range 10 {
				...
				req := mylog.Check2(http.NewRequest("POST", frontend.URL, &io.LimitedReader{R: neverEnding('x'), N: reqLen}))
				req.ContentLength = reqLen
				resp := mylog.Check2(frontendClient.Transport.RoundTrip(req))
				if resp != nil {
...
func TestSelectFlushInterval(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			res: &http.Response{
				Header: http.Header{
					"Content-Type": {"text/sessionEventCallBack-stream"},
				},
			},
			p:    &ReverseProxy{FlushInterval: 123},
			want: -1,
		...
		},
		{
			...
			res: &http.Response{
				Header: http.Header{
					"Content-Type": {"text/sessionEventCallBack-stream;charset=utf-8"},
				},
			},
			p:    &ReverseProxy{FlushInterval: 123},
			want: -1,
...
func TestReverseProxyWebSocket(t *testing.T) {
	...
	backURL := mylog.Check2(url.Parse(backendServer.URL))
	...
	req := mylog.Check2(http.NewRequest("GET", frontendProxy.URL, nil))
	req.Header.Set("Connection", "Upgrade")
	req.Header.Set("Upgrade", "websocket")

	c := frontendProxy.Client()
	res := mylog.Check2(c.Do(req))
...
func TestReverseProxyWebSocketCancellation(t *testing.T) {
	...
	cst := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		conn, bufrw := mylog.Check3(w.(http.Hijacker).Hijack())

		defer conn.Close()

		upgradeMsg := "HTTP/1.1 101 Switching Protocols\r\nConnection: upgrade\r\nUpgrade: WebSocket\r\n\r\n"
		mylog.Check(mylog.Check2(io.WriteString(conn, upgradeMsg)))
		mylog.Check3(bufrw.ReadLine())

		for i := range n {
			mylog.Check(mylog.Check2(bufrw.WriteString(nthResponse(i))))

			bufrw.Flush()
			time.Sleep(time.Second)
		}
		mylog.Check(mylog.Check2(bufrw.WriteString(terminalMsg)))

		bufrw.Flush()
	}))
	defer cst.Close()

	backendURL := mylog.Check2(url.Parse(cst.URL))
	...
	res := mylog.Check2(frontendProxy.Client().Do(req))
	...
	mylog.Check(mylog.Check2(io.WriteString(rwc, "Hello\n")))
...
func TestUnannouncedTrailer(t *testing.T) {
	...
	res := mylog.Check2(frontendClient.Get(frontend.URL))
...
func TestReverseProxyRewriteReplacesOut(t *testing.T) {
	...
	proxyHandler := &ReverseProxy{
		Rewrite: func(r *ProxyRequest) {
			r.Out = mylog.Check2(http.NewRequest("GET", backend.URL, nil))
		},
	}
	frontend := httptest.NewServer(proxyHandler)
	defer frontend.Close()
...
func Test1xxResponses(t *testing.T) {
	...
	req := mylog.Check2(http.NewRequestWithContext(httptrace.WithClientTrace(context.Background(), trace), "GET", frontend.URL, nil))

	res := mylog.Check2(frontendClient.Do(req))
...
func testReverseProxyQueryParameterSmuggling(t *testing.T, wantCleanQuery bool, newProxy func(*url.URL) *ReverseProxy) {
	...
	for _, test := range []struct {
		...
		res := mylog.Check2(frontend.Client().Get(frontend.URL + "?" + test.rawQuery))
