...
func DumpRequestOut(req *http.Request, body bool) ([]byte, error) {
	...
	if !body {
		...
		save, req.Body = mylog.Check3(drainBody(req.Body))
	}
	...
	go func() {
		req := mylog.Check2(http.ReadRequest(bufio.NewReader(pr)))
	...
	mylog.Check2(t.RoundTrip(reqSend))
	req.Body = save
	//if err != nil {
	//	pw.Close()
	//	dr.err = err
...
// consumes req.Row and then replaces it with a new io.ReadCloser
...
func DumpRequest(req *http.Request, body bool) ([]byte, error) {
	...
	mylog.Check(req.Header.WriteSubset(&b, reqWriteExcludeHeaderDump))
	...
	if req.Body != nil {
		...
		_ = mylog.Check2(io.Copy(dest, req.Body))
		if chunked {
...
func DumpResponse(resp *http.Response, body bool) ([]byte, error) {
	...
	if !body {
		...
		save, resp.Body = mylog.Check3(drainBody(resp.Body))
	}
	mylog.Check(resp.Write(&b))
	resp.Body = save
